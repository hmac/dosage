...
module Gentamicin exposing (Model, init, view)
...
view : (Model -> msg) -> Model -> Html.Html msg
view update model =
    let
        obese =
        ...
        baseInputs =
            [ ( "Sex", sexInput update )
            , ( "Height", heightInput update )
            , ( "Age", ageInput update )
            , ( "Weight", weightInput update )
            , ( "Serum Creatinine", serumCreatinineInput update )
            , ( "Dosage", dosageInput update )
            ]

        inputs =
            if model.dosage == Daily7mg then
                baseInputs
                    ++ [ ( "Gentamicin", gentamicinLevelInput update ) ]
...
setHeightValue : Model -> HeightUnit -> String -> Model
setHeightValue model unit heightStr =
    { model | height = Maybe.map (Height unit) (String.toInt heightStr) }


setHeightUnit : Model -> String -> Model
setHeightUnit model unitStr =
...
setWeight : Model -> String -> Model
setWeight model weightStr =
    { model | weight = Maybe.map Weight (String.toInt weightStr) }


setAge : Model -> String -> Model
setAge model ageStr =
    { model | age = Maybe.map Age (String.toInt ageStr) }


setSerumCreatinine : Model -> String -> Model
setSerumCreatinine model scStr =
    { model | serumCreatinine = Maybe.map SerumCreatinine (String.toFloat scStr) }


setDosage : Model -> Maybe Dosage -> Model
setDosage model mDosage =
    case mDosage of
        Just d ->
            { model | dosage = d }
...
setGentamicinMeasurement : Model -> Maybe Int -> Maybe Float -> Model
setGentamicinMeasurement model mHour mLevel =
    case mHour of
        Nothing ->
            model
...
-- Inputs
-- want: msg
-- setSex : Model -> a -> Model
-- update : Model -> msg
-- setSex model : a -> Model
-- (\a -> update (setSex Model a)) : a -> msg
-- update' : (Model -> msg) -> (a -> Model) -> (a -> msg)
-- update' update f = update << f
...
heightInput : (Model -> msg) -> Model -> Html.Html msg
heightInput update model =
    let
        unit =
            Maybe.withDefault model.defaultHeightUnit (Maybe.map (\(Height u _) -> u) model.height)
...
weightInput : (Model -> msg) -> Model -> Html.Html msg
weightInput update model =
    let
        weightStr =
...
ageInput : (Model -> msg) -> Model -> Html.Html msg
ageInput update model =
    let
        ageStr =
...
serumCreatinineInput : (Model -> msg) -> Model -> Html.Html msg
serumCreatinineInput update model =
    let
        scStr =
...
dosageInput : (Model -> msg) -> Model -> Html.Html msg
dosageInput update model =
    let
        dosage =
            model.dosage
    in
...
gentamicinLevelInput : (Model -> msg) -> Model -> Html.Html msg
gentamicinLevelInput update model =
    let
        ( hour, mLevel ) =
            model.gentamicinMeasurement
    in
    case mLevel of
        Nothing ->
            let
                hourHandler =
                    map (\hStr -> update (setGentamicinMeasurement model (S.toInt hStr) Nothing)) targetValue
        ...
        Just level ->
            let
                hourHandler =
                    map
                        (\hStr -> update (setGentamicinMeasurement model (S.toInt hStr) (Just level)))
                        targetValue
